...
Navigate to the Pub/sub project directory:

```bash
cd pub_sub/python/checkout
```
...
```bash
cd pub_sub/python/order-processor
```
...
```bash
cd pub_sub/javascript/checkout
```
...
```bash
cd pub_sub/javascript/order-processor
```
...
In a terminal window, navigate to the Pub/sub project directory:

```bash
cd pub_sub/csharp/checkout
```
...
```bash
dapr run --app-id checkout --app-port 6002 --dapr-http-port 3602 --components-path ../../components -- mvn spring-boot:run
```
...
```bash
dapr run --app-id order-processor --app-port 6001 --dapr-http-port 3601 --components-path ../../components -- mvn spring-boot:run
...
```bash
cd pub_sub/java/order-processor
```
